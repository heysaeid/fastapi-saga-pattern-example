...
async def test_create_order(create_order):
    ...
    assert created_order.customer_id == 123
    assert created_order.total_amount == 400.0
    assert len(created_order.order_items) == 2
    assert created_order.order_items[0].product_id == 1
    assert created_order.order_items[1].product_id == 2
